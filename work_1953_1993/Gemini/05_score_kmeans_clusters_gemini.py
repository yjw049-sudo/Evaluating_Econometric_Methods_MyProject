...
Usage:
    ...
    python 05_score_kmeans_clusters_gemini.py --input ../output/Kmeans/cluster_summary.csv
"""
...
DEFAULT_INPUT_PATH = ROOT.parent / "output" / "Kmeans" / "cluster_summary.csv"
  ...
  "clusters": [
    {
      ...
      "rationale": "<brief explanation based on the top words>"
    }
  ]
}
"""
    ...
    "properties": {
        "clusters": {
            ...
            "items": {
                ...
                "required": ["cluster", "procedural_score", "rationale"],
            },
        },
    },
    "required": ["clusters"],
...
def build_prompt(df: pd.DataFrame) -> str:
    ...
    for _, row in df.iterrows():
        lines.append(f"cluster={int(row['cluster'])} | top_words={row['top_words']}")
...
def main() -> None:
    ...
    out = out[["cluster", "top_words", "procedural_score", "rationale"]]
    out = out.sort_values("cluster").reset_index(drop=True)
    ...
    for _, row in out.iterrows():
        print(f"cluster {int(row['cluster']):>2}: {row['procedural_score']}  ({row['rationale']})")
