...
INPUT_PATH = ROOT / "speeches_Gemin_s0.csv"
...
TEXT_COLUMN = "speechtext_chunk"
TEXT_COLUMN_ALIASES = ["speechtext_chunk"]
ID_COLUMNS = [BASEPK_COLUMN, CHUNK_INDEX_COLUMN]
...
OUT_FIELDS = ["basepk", "chunk_index", "topic", "procedural_score", "error"]
COMPLETED_STATES = {
    "JOB_STATE_SUCCEEDED",
    "JOB_STATE_FAILED",
    "JOB_STATE_CANCELLED",
...
SYSTEM_INSTRUCTION = """\
You are a research assistant analysing speech chunks from the Canadian House of Commons, 1963-1993.

For each speech chunk, return valid JSON with two fields:
...
def read_input_csv(path: Path) -> pd.DataFrame:
    ...
    if missing_id_columns:
        sys.exit(f"Required ID columns not found: {missing_id_columns}. Available columns: {list(df.columns)}")
    ...
    df = df[ID_COLUMNS + [text_col]].rename(columns={text_col: TEXT_COLUMN}).copy()
    df = df[df[BASEPK_COLUMN].notna() & df[CHUNK_INDEX_COLUMN].notna()].copy()
    ...
    df[CHUNK_INDEX_COLUMN] = df[CHUNK_INDEX_COLUMN].astype(int)
    df[TEXT_COLUMN] = df[TEXT_COLUMN].astype(str)
    return df.reset_index(drop=True)


def load_done(path: Path) -> set[tuple[int, int]]:
    """Return only chunk identifiers with a complete, error-free result."""
    if not path.exists():
        return set()
    try:
        df = pd.read_csv(path, encoding="utf-8-sig")
        required = [*ID_COLUMNS, "topic", "procedural_score"]
        if any(column not in df.columns for column in required):
            return set()

        valid = df[ID_COLUMNS].notna().all(axis=1)
        valid &= df["topic"].fillna("").astype(str).str.strip().ne("")
        valid &= pd.to_numeric(df["procedural_score"], errors="coerce").notna()
        if "error" in df.columns:
            valid &= df["error"].fillna("").astype(str).str.strip().eq("")

        done = df.loc[valid, ID_COLUMNS]
        return set(
            zip(
                done[BASEPK_COLUMN].astype(int),
                done[CHUNK_INDEX_COLUMN].astype(int),
            )
        )
    except Exception:
        return set()
...
def exclude_done(df: pd.DataFrame, done: set[tuple[int, int]]) -> pd.DataFrame:
    """Remove rows whose (basepk, chunk_index) identifier is already complete."""
    if not done:
        return df
    row_ids = pd.MultiIndex.from_frame(df[ID_COLUMNS])
    done_ids = pd.MultiIndex.from_tuples(done, names=ID_COLUMNS)
    return df.loc[~row_ids.isin(done_ids)].reset_index(drop=True)
...
def build_prompt(row: pd.Series) -> str:
    ...
    return PROMPT_TEMPLATE.format(
        basepk=row[BASEPK_COLUMN],
        chunk_index=row[CHUNK_INDEX_COLUMN],
        speechtext=text,
    )
...
def write_jsonl_for_df(df: pd.DataFrame, jsonl_path: Path) -> int:
    ...
    with open(jsonl_path, "w", encoding="utf-8") as f:
        for _, row in df.iterrows():
            ...
            chunk_index = int(row[CHUNK_INDEX_COLUMN])
            record = {
                "key": f"{basepk}_{chunk_index}",
...
def make_jsonl(args: argparse.Namespace) -> None:
    ...
    if done:
        print(f"Skipping {len(done):,} chunks already in {args.output.name}")
        df = exclude_done(df, done)
    ...
    if args.limit is not None:
        ...
        print(f"Limit applied: {len(df):,} speech chunks")
...
def make_chunks(args: argparse.Namespace) -> None:
    ...
    if args.limit is not None:
        df = df.head(args.limit).copy()
        print(f"Global limit applied: {len(df):,} speech chunks")
...
def parse_result_line(line: str) -> dict[str, Any]:
    ...
    key_parts = str(key).rsplit("_", maxsplit=1)
    if len(key_parts) != 2:
        raise ValueError(f"Invalid chunk key '{key}'; expected '<basepk>_<chunk_index>'")
    basepk, chunk_index = map(int, key_parts)
    if error and not response:
        return {"basepk": basepk, "chunk_index": chunk_index, "topic": "", "procedural_score": "", "error": json.dumps(error, ensure_ascii=False)[:300]}
    text = extract_text_from_response(response)
    if not text:
        return {"basepk": basepk, "chunk_index": chunk_index, "topic": "", "procedural_score": "", "error": "empty response text"}
    try:
        topic, score = parse_model_json(text)
        return {"basepk": basepk, "chunk_index": chunk_index, "topic": topic, "procedural_score": score, "error": ""}
    except Exception as exc:
        return {"basepk": basepk, "chunk_index": chunk_index, "topic": "", "procedural_score": "", "error": f"ParseError: {exc}: {text[:200]}"}
    ...
    with open(args.results, "r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            try:
                ...
                rows.append({"basepk": "", "chunk_index": "", "topic": "", "procedural_score": "", "error": f"Line {line_no}: {exc}"})

    rows = sorted(
        rows,
        key=lambda r: (
            r["basepk"] == "",
            int(r["basepk"]) if r["basepk"] != "" else 10**18,
            int(r["chunk_index"]) if r["chunk_index"] != "" else 10**18,
        ),
    )
...
def merge_parts(args: argparse.Namespace) -> None:
    ...
    for file in files:
        ...
        if any(column not in df.columns for column in ID_COLUMNS):
            print(f"Skipping {file}: missing one or more ID columns {ID_COLUMNS}")
            continue
        frames.append(df)
    ...
    out = out.sort_values(ID_COLUMNS).reset_index(drop=True)
    args.output.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    p.add_argument("--include-done", action="store_true", help="do not skip (basepk, chunk_index) pairs already in final output")
