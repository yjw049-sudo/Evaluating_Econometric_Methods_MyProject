...
You are a research assistant building a semantic taxonomy for speeches in the Canadian House of Commons, 1963-1993. You will be shown a list of up to {top_n} short topic descriptions, each with a frequency count.
...
def top_topics(path: Path, n: int) -> tuple[pd.Series, dict[str, Any]]:
    ...
    print(f"{len(df):,} speech chunks; {len(counts):,} unique topics")
    print(f"Top {k}: covers {counts.head(k).sum():,} speech chunks ({100 * coverage:.1f}%)")

    metadata = {
        "input_path": str(path),
        "n_speech_chunks_with_topic": int(len(df)),
